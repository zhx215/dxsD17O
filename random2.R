...
mixing_computer_1b <- function(d18O_input,dxs_input,D17O_input,td_input,td_final,t_input,t_final,vmt,mit,lambda,n,td_step){
  dd <- sample(1:12,1)
  t_step <- td_step*(t_input-t_final)/(td_input-td_final)
  
  # ignores small rainout situations
  if ((td_input-td_step) < td_final) {
    d17O_input <- get_d17O(d18O_input,D17O_input)
    d2H_input <- dxs_input+8*d18O_input
    result <- Rayleigh_computer_1b(d18O_input,dxs_input,D17O_input,td_input,td_final,t_input,t_final,n,T,vmt,mit,lambda,T,dd)
    return(result)
  }
  
  # large rainout situations
  repeat{
    d17O_input <- get_d17O(d18O_input,D17O_input)
    d2H_input <- dxs_input+8*d18O_input
    if ((td_input-td_step) < td_final) {
      result <- Rayleigh_computer_1b(d18O_input,dxs_input,D17O_input,td_input,td_input-td_step,t_input,t_input-t_step,n,T,vmt,mit,lambda,T,dd)
      if (length(result)==1) {return(NA)}
      return(result[c(7,8,9,4,5,6)])
    }
    result <- Rayleigh_computer_1b(d18O_input,dxs_input,D17O_input,td_input,td_input-td_step,t_input,t_input-t_step,n,T,vmt,mit,lambda,T,dd)
    if (length(result)==1) {return(NA)}
    ...
    td_mix <- weighted.mean(c(td_input,td_input-td_step),
                            c(CC_relation(td_input),CC_relation(td_input-td_step)))
    t_mix <- weighted.mean(c(t_input,t_input-t_step),
                           c(CC_relation(td_input),CC_relation(td_input-td_step)))
    d18O_input <- d18O_mix
    dxs_input <- dxs_mix
    D17O_input <- D17O_mix
...
load("C:\\Users\\zhyxi\\Desktop\\17O project\\RR and rh_5.RData")
# 1
...
# }
# save.image("C:/Users/zhyxi/Desktop/17O project/SEP simulation 6.RData")

load("C:/Users/zhyxi/Desktop/17O project/SEP simulation 6.RData")
# a
...
text(-26,45.5,"(a) Stochastic simulation #4",cex=1,pos=4)
text(-25,32,"Rayleigh distillation +\nraindrop re-evaporation w/\nequilibration + vapor mixing",cex=1,pos=4)
...
dxs_output4 <- dxsp_result[id]
D17O_output4 <- D17Op_result_wer[id]

# 2
D17Op_result_wer <- vector()
for (i in 1:length(D17Op_result)){
  D17Op_result_wer[i] <- rnorm(1,D17Op_result[i],10)
}
# a
plot(NA,NA,xlim=c(-25,5),ylim=c(-21,42),xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="")
axis(1,seq(-25,5,5),cex.axis=0.9,padj=-1.3,tck=-0.02)
...
text(-26,45.5,"(b) Stochastic simulation #5",cex=1,pos=4)
text(-25,32,"Rayleigh distillation +\nraindrop re-evaporation w/\nequilibration + vapor mixing +",cex=1,pos=4)
text(-25,24,expression(paste("larger ",Delta*minute^17,"O"," error")),cex=1,pos=4)
...
coef <- odregress(dxsp_result[id],D17Op_result_wer[id])$coef
lines((c(-42,82)-coef[2])/coef[1],c(-42,82))
dxs_output5 <- dxsp_result[id]
D17O_output5 <- D17Op_result_wer[id]
...
#     dxsv_source <- rnorm(1,-0.54*rhs_source+48.2,4.35)
#     D17Ov_source <- rnorm(1,-0.74*rhs_source+65.3,5.6)
#     result <- Rayleigh_computer_1(d18Ov_source,dxsv_source,D17Ov_source,td_source,td_sink,100,T,-10+273.15,-40+273.15,0.004)
#     d18Op_result <- c(d18Op_result,result[1])
#     d17Op_result <- c(d17Op_result,result[2])
#     d2Hp_result <- c(d2Hp_result,result[3])
#     td_sink_result <- c(td_sink_result,td_sink)
...
#   D17Op_result_wer[i] <- rnorm(1,D17Op_result[i],5)
# }
# save.image("C:/Users/zhyxi/Desktop/17O project/SEP simulation 7.RData")

load("C:/Users/zhyxi/Desktop/17O project/SEP simulation 7.RData")
# a
plot(NA,NA,xlim=c(-25,5),ylim=c(-21,42),xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="")
axis(1,seq(-25,5,5),cex.axis=0.9,padj=-1.3,tck=-0.02)
axis(2,seq(-20,40,10),cex.axis=0.9,padj=1.1,tck=-0.02)
...
text(-26,45.5,"(c) Stochastic simulation #6",cex=1,pos=4)
text(-25,35,"Rayleigh distillation w/\ndelayed ice-vapor fractionation",cex=1,pos=4)
...
coef <- odregress(dxsp_result[id],D17Op_result_wer[id])$coef
lines((c(-42,82)-coef[2])/coef[1],c(-42,82))
dxs_output6 <- dxsp_result[id]
D17O_output6 <- D17Op_result_wer[id]